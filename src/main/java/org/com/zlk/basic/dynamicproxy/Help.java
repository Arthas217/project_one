package org.com.zlk.basic.dynamicproxy;

/**
 * @Author 会游泳的蚂蚁
 * @Description: 动态代理参考地址
 * CGLIB像是一个拦截器，在调用我们的代理类方法时，代理类(子类)会去找到目标类(父类),此时它会被一个方法拦截器所拦截，在拦截器中才会去实现方法的调用。并且还会对方法进行行为增强。
 * https://www.cnblogs.com/blessing2022/p/16622146.html
 * https://blog.csdn.net/m0_61609890/article/details/123420091
 * https://blog.csdn.net/NCS123456/article/details/125225705
 * https://bugstack.cn/md/java/core/2019-12-21-%5B%E6%9C%89%E7%82%B9%E5%B9%B2%E8%B4%A7%5DJDK%E3%80%81CGLIB%E5%8A%A8%E6%80%81%E4%BB%A3%E7%90%86%E4%BD%BF%E7%94%A8%E4%BB%A5%E5%8F%8A%E6%BA%90%E7%A0%81%E5%88%86%E6%9E%90.html
 * @Date 2022/11/10 17:30
 */
public interface Help {
   //todo
}
