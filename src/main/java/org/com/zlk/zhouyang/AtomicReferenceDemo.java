...
//        ABA();
    ...
    private static void solveABA() {
        new Thread(() -> {
            ...
            System.out.println("---cc第二次---" + atomicstampref.compareAndSet(100, 101,
                    atomicstampref.getStamp(), atomicstampref.getStamp() + 1));
            System.out.println(Thread.currentThread().getName() + "\t 第二次版本号" + atomicstampref.getStamp());

            System.out.println("---cc第三次---" +atomicstampref.compareAndSet(101, 100,
                    atomicstampref.getStamp(), atomicstampref.getStamp() + 1));
            System.out.println(Thread.currentThread().getName() + "\t 第三次版本号" + atomicstampref.getStamp());
        }, "CC").start();
    ...
    private static void ABA() {
        ...
        new Thread(() -> {
            ...
            System.out.println(atomicref.compareAndSet(100, 2020) + " data：" + atomicref.get());
        }, "BB").start();
    }
}
